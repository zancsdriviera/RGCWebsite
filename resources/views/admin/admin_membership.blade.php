...
@section('content')
                        ...
                        @forelse ($contents as $c)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge
                                    @if ($c->type === 'download') bg-info text-dark
                                    @elseif($c->type === 'members_data') bg-warning text-dark
                                ...
                                <td>
                                    @if ($c->file_path)
                                        @if (Str::endsWith($c->file_path, ['.pdf']))
                                            <a href="{{ asset('storage/' . $c->file_path) }}" target="_blank"
                                                class="btn btn-sm btn-outline-primary">
                                                <i class="bi bi-file-pdf me-1"></i>View PDF
                                            </a>
                                        @else
                                            <img src="{{ asset('storage/' . $c->file_path) }}" class="img-thumbnail"
                                                width="70" alt="Preview">
                                        @endif
                                    @elseif($c->type === 'bank')
                                        <div class="d-flex justify-content-center gap-2">
                                            @if ($c->top_image)
                                                <img src="{{ asset('storage/' . $c->top_image) }}" class="img-thumbnail"
                                                    width="50" alt="Logo">
                                            @endif
                                            @if ($c->qr_image)
                                                <img src="{{ asset('storage/' . $c->qr_image) }}" class="img-thumbnail"
                                                    width="50" alt="QR">
                                            @endif
                                        </div>
                                    @else
                                ...
                                <td>
                                    ...
                                    @endphp
                                    {{ $sz > 0 ? number_format($sz / 1024, 1) . ' KB' : '—' }}
                                </td>
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-outline-primary edit-btn" data-id="{{ $c->id }}"
                                            data-type="{{ $c->type }}" data-title="{{ $c->title ?? '' }}"
                                            data-file="{{ $c->file_path ?? '' }}" data-top="{{ $c->top_image ?? '' }}"
                                            data-qr="{{ $c->qr_image ?? '' }}" data-bs-toggle="modal"
                                            data-bs-target="#editModal">
                                            <i class="bi bi-pencil-square"></i> Edit
                                        </button>
                                        <button class="btn btn-outline-danger delete-btn"
                                            data-url="{{ route('admin.membership.destroy', $c->id) }}"
    ...
    ══════════════════════════════════════════════ --}}
        <div class="card mb-4">
            ...
            <div class="card-body table-responsive p-0">
                <form id="bulkDeleteForm" action="{{ route('admin.membership.applications.bulkDestroy') }}"
                    method="POST">
                    @csrf
                    <table class="table table-bordered table-striped align-middle text-center mb-0" id="appTable">
                        ...
                        <tbody id="appTableBody">
                            @forelse ($applications as $app)
                                <tr class="app-row"
                                    data-name="{{ strtolower($app->family_name . ' ' . $app->given_name . ' ' . ($app->middle_name ?? '')) }}"
                                    data-email="{{ strtolower($app->email) }}">
                                    <td><input type="checkbox" name="ids[]" value="{{ $app->id }}"
                                            class="form-check-input app-checkbox"></td>
                                    <td>{{ $app->id }}</td>
                                    <td class="text-start fw-semibold">{{ $app->family_name }},
                                        {{ $app->given_name }}{{ $app->middle_name ? ' ' . $app->middle_name : '' }}</td>
                                    <td class="text-start">{{ $app->email }}</td>
                                    <td>
                                        <span
                                            class="badge
                                        @if ($app->class_of_membership === 'A') bg-primary
                                        @elseif($app->class_of_membership === 'B') bg-info text-dark
                                        @else bg-success @endif">
                                            {{ $app->class_of_membership ? '"' . $app->class_of_membership . '" Share' : '—' }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <button type="button" class="btn btn-outline-success view-app-btn"
                                                data-id="{{ $app->id }}"
                                                data-url="{{ route('admin.membership.applications.view', $app->id) }}">
                                                <i class="bi bi-eye"></i> View
                                            </button>
                                            <button type="button" class="btn btn-outline-danger delete-app-btn"
                                                data-url="{{ route('admin.membership.applications.destroy', $app->id) }}"
            ...
            document.querySelectorAll('.edit-btn').forEach(btn => {
                ...
                <div class="mb-3">
                    ...
                    <input type="file" name="file_path" class="form-control" accept=".pdf" data-file-type="pdf">
                        ...
                        html = `
                <div class="mb-3">
                    ${file ? `<img src="{{ asset('storage') }}/${file}" class="img-thumbnail mb-2" width="180">` : '<p class="text-muted small">No image uploaded</p>'}
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6">
                        ...
                        ${top ? `<img src="{{ asset('storage') }}/${top}" class="img-thumbnail w-100">` : '<p class="text-muted small">None</p>'}
                    </div>
                    <div class="col-6">
                        <label class="form-label fw-semibold small">Current QR</label>
                        ${qr ? `<img src="{{ asset('storage') }}/${qr}" class="img-thumbnail w-100">` : '<p class="text-muted small">None</p>'}
                    </div>
                </div>
                <div class="row g-3">
                    <div class="col-6">

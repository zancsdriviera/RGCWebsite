    ...
    <div class="container-fluid p-4">
        ...
        <form method="GET" class="row g-2 mb-3 align-items-end">
            ...
            <div class="col-auto">
                ...
                <a href="{{ route('admin.coursesched.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        ...
        <div class="table-responsive bg-white p-2 rounded">
            <table class="table table-bordered align-middle text-center table-striped">
                ...
                <tbody>
                    @forelse($events as $e)
                        <tr>
                            <td>{{ \Carbon\Carbon::parse($e->date)->format('F d, Y') }}</td>

                            <td>
                                @if ($e->langer_status === 'Others')
                                    {{ $e->langer_other ?? 'TBA' }}
                                @else
                                    {{ $e->langer_status ?? 'TBA' }}
                                @endif
                            ...
                            <td>
                                @if ($e->couples_status === 'Others')
                                    {{ $e->couples_other ?? 'TBA' }}
                                @else
                                    {{ $e->couples_status ?? 'TBA' }}
                                @endif
                            </td>

                            <td>
                                <button type="button" class="btn bg-primary text-white btn-sm editBtn"
                                    data-id="{{ $e->id }}" data-date="{{ $e->date->toDateString() }}"
                                    data-langer-status="{{ $e->langer_status }}"
                                    data-langer-other="{{ $e->langer_other }}"
                                    data-couples-status="{{ $e->couples_status }}"
                                    data-couples-other="{{ $e->couples_other }}" data-bs-toggle="modal"
                                    data-bs-target="#editModal">
                                    <i class="bi bi-pencil-square"></i> Edit
                                </button>

                                <button type="button" class="btn btn-danger btn-sm deleteBtn"
                                    data-id="{{ $e->id }}" data-bs-toggle="modal" data-bs-target="#deleteModal">
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <form action="{{ route('admin.coursesched.store') }}" method="POST" id="addForm">
                    @csrf

...
@section('content')
    ...
    <div class="container-fluid px-4 py-3">
                        ...
                        <form method="POST" action="{{ route('admin.settings.update') }}" class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" name="lean_season_caption" class="form-control"
                                    value="{{ $leanSeasonCaption ?? 'LEAN SEASON (APRIL - OCTOBER 2025)' }}" required>
        ...
        <table class="table table-bordered align-middle text-center table-striped">
            ...
            <tbody>
                @forelse($gleans as $glean)
                    <tr>
                        <td>{{ ucfirst($glean->type) }}</td>
                        <td>{{ $glean->title1 ?? ($glean->title2 ?? $glean->title3) }}</td>
                        <td>{{ $glean->sched1 ?? $glean->sched2 }}</td>
                        <td>
                            <!-- Edit Button -->
                            <button class="btn btn-outline-primary btn-sm" data-bs-toggle="modal"
                                data-bs-target="#editModal" data-id="{{ $glean->id }}" data-type="{{ $glean->type }}"
                                ...
                                data-paragraph2="{{ $glean->paragraph2 ?? '' }}" data-total2="{{ $glean->total2 ?? '' }}"
                                data-body2="{{ $glean->body2 ?? '' }}" data-price2="{{ $glean->price2 ?? '' }}"
                                data-sched2="{{ $glean->sched2 ?? '' }}" data-title3="{{ $glean->title3 ?? '' }}"
                                data-paragraph3="{{ $glean->paragraph3 ?? '' }}" data-body3="{{ $glean->body3 ?? '' }}"
                                data-price3="{{ $glean->price3 ?? '' }}">
    ...
    <div class="modal fade" id="addModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form method="POST" action="{{ route('admin.glean.store') }}">
                    @csrf

                ...
                <div class="rgc-section-block">
                    ...
                    @foreach ($titles as $item)
                        <div class="rgc-section-heading">
                            ...
                            @endforeach
                            @if ($item->description)
                                <p class="rgc-sub">{{ $item->description }}</p>
                            @endif
                                ...
                                <div class="col-12 col-md-6 col-lg-5 mb-4 d-flex">
                                    <article class="rgc-card w-100">
                                        ...
                                        <div class="rgc-card-header">
                                            ...
                                            <div class="rgc-title">
                                                @foreach ($titleLines as $line)
                                                    <span style="display:block;">{{ trim($line) }}</span>
                                                @endforeach
                                            </div>
                                            @if ($gpeak->gr_title_description)
                                                <div class="rgc-subtitle">{{ $gpeak->gr_title_description }}</div>
                                            @endif
                                        </div>
                                        ...
                                        @if ($gpeak->gr_total)
                                            <div class="rgc-card-total">
                                                ...
                                                <div class="rgc-total-amount">₱{{ number_format($gpeak->gr_total, 2) }}
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        <div class="rgc-card-body">
                                            ...
                                            @if ($gpeak->gr_content)
                                                ...
                                                <ul class="rgc-fee-list">
                                                    @foreach ($bodies as $index => $body)
                                                        @if (trim($body) !== '')
                                                            <li>
                                                                <span class="rgc-fee-label">{{ trim($body) }}</span>
                                                                @if (isset($prices[$index]) && trim($prices[$index]) !== '')
                                                                    <span
                                                                        class="rgc-fee-amount">₱{{ number_format((float) trim($prices[$index]), 2) }}</span>
                                                                @endif
                                                            </li>
                                                        @endif
                                                    @endforeach
                                            ...
                                            @if ($gpeak->gr_schedule)
                                                <div class="rgc-schedule">
                                                    <button class="rgc-schedule-btn">{{ $gpeak->gr_schedule }}</button>
                                                </div>
                                            @endif

                                            @if ($gpeak->gr_description)
                                                <p class="rgc-card-desc">{{ $gpeak->gr_description }}</p>
                                            @endif

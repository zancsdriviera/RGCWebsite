...
@section('content')
    @if ($homepage)
            ...
            <div id="mainCarousel" class="carousel slide" data-bs-ride="false">
                <div class="carousel-inner">
                    ...
                    @foreach ($dynamicCarousels as $carousel)
                        ...
                        @endphp
                        <div class="carousel-item dynamic-carousel-slide {{ $isVideo ? 'has-video' : '' }}"
                            data-caption-anim="float-up">

                            @if ($isVideo)
                                {{-- Inline video: clean, no controls, muted, plays only when slide is active --}}
                                <video class="d-block w-100 inline-carousel-video" style="height:100vh; object-fit:cover;"
                                    muted playsinline data-src="{{ asset('storage/' . $carousel['image']) }}"
                                    data-mime="{{ $mime }}">
                                </video>

                                {{-- Bottom-right buttons: sound + zoom --}}
                                <div
                                    ...
                                    <button class="btn btn-dark btn-sm video-zoom-btn"
                                        style="opacity:0.75; border-radius:50%; width:42px; height:42px; padding:0;"
                                        data-src="{{ asset('storage/' . $carousel['image']) }}"
                                        data-mime="{{ $mime }}" title="Zoom video">
        ...
        {{-- Cards, headline, subheadline, map, contact info --}}
...
@push('scripts')
    {{-- Caption animation script (unchanged) --}}
    <script>
        (function() {

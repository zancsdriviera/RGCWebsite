...
class AdminDrivingRangeController extends Controller
{
    // Show admin CMS page
    public function index()
    {
        ...
        return view('admin.admin_drivingrange', compact('description', 'images'));
    }
    ...
    public function updateDescription(Request $request)
    {
        ...
        $content = DrivingrangeContent::firstOrNew(['id' => 1]); // always one description
    ...
    public function uploadImages(Request $request)
    {
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $image) {
                $path = $image->store('drivingrange', 'public');
                DrivingrangeContent::create(['image_path' => '/storage/' . $path]);
            }
        }
    ...
    public function updateImage(Request $request, $id)
    {
        ...
        $img = DrivingrangeContent::findOrFail($id);
        ...
        $path = $request->file('image')->store('drivingrange', 'public');
    ...
    {
        $image = DrivingrangeContent::findOrFail($id);
